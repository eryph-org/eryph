...
public readonly record struct StorageNames
{
        ...
        Func<VmHostAgentDefaultsConfiguration, string> getDefault)
    {
        return match(
            ...
            from storageIdentifier in match(
                Optional(Path.HasExtension(remainingPath) ? Path.GetDirectoryName(remainingPath) : remainingPath)
                    .Filter(notEmpty),
                s => from si in StorageIdentifier.NewOption(s)
                    select Some(si.Value),
                () => Some(Option<string>.None))
            select (
                new StorageNames
                {
                ...
                storageIdentifier),
            v => v,
            () => (new StorageNames { DataStoreName = None, EnvironmentName = None, ProjectName = None }, None));
    }
    ...
    {
        var names = this;
        return from dsName in names.DataStoreName.ToEither(
            ...
            from environmentName in names.EnvironmentName.ToEither(
                Error.New("Unknown environment name. Cannot resolve path"))
            from datastorePaths in LookupVMDatastorePathInEnvironment(dsName, environmentName, vmHostAgentConfig)
            select (JoinPathAndProject(datastorePaths.VmPath, projectName),
                JoinPathAndProject(datastorePaths.VhdPath, projectName));
    }
    ...
    private static Option<(Option<ProjectName> Name, Option<Guid> Id)> GetProjectNameOrId(string root)
    {
        ...
        return ConfigModel.ProjectName.NewOption(name).Match(
            n => Option<(Option<ProjectName> Name, Option<Guid> Id)>.Some((n, None)),
            () => Option<(Option<ProjectName> Name, Option<Guid> Id)>.None);
    }
}
