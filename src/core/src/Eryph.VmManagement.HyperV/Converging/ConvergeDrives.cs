...
public class ConvergeDrives(ConvergeContext context) : ConvergeTaskBase(context)
{
    ...
    private EitherAsync<Error, Unit> DetachUndefinedHardDrives(
        ...
        from _2 in ConvergeHelpers.FindAndApply(vmInfo,
            ...
            device =>
            {
                ...
                switch (detach)
                {
                    case true when frozenDiskIds.Contains(hd.Id):
    ...
    private static PsCommandBuilder BuildAttachCommand(TypedPsObject<VirtualMachineInfo> vmInfo, string vhdPath,
        VMDriveStorageSettings driveSettings)
    {
