...
public partial class SupportRequestPage
{
    ...
    public SupportRequestPage(string issueType, string issueDetails, string email)
    {
        ...
        if (!string.IsNullOrWhiteSpace(issueDetails)) summary += $"\n\nDetails:\n{issueDetails}";
        summary += $"\n\nContact email: {email}";
            ...
            var json = $$"""
                         {
                             ...
                             "properties": {
                                 ...
                                 "issue_type": "{{HttpUtility.JavaScriptStringEncode(_issueType)}}",
                                 "issue_details": "{{HttpUtility.JavaScriptStringEncode(_issueDetails)}}",
                                 "contact_email": "{{HttpUtility.JavaScriptStringEncode(_email)}}",
                                 "source": "uninstaller",
