...
internal class RemoveProjectMemberCommandHandler(
    ...
    ITaskMessaging messaging) : IHandleMessages<OperationTask<RemoveProjectMemberCommand>>
{
    public async Task Handle(OperationTask<RemoveProjectMemberCommand> message)
    {
        ...
        var assignment = await stateStore.For<ProjectRoleAssignment>()
            .GetBySpecAsync(
        ...
        if (assignment != null)
        {
            ...
            await stateStore.For<ProjectRoleAssignment>().DeleteAsync(assignment, stoppingToken.Token);
        }
