...
internal class IpPoolManager : IIpPoolManager
{
    ...
    public EitherAsync<Error,IpPoolAssignment> AcquireIp(Guid subnetId, string poolName, CancellationToken cancellationToken)
    {
        return Prelude.TryAsync(async () =>
        {
            ...
            var startNumber = IPNetwork.ToBigInteger(firstIp);
            var endNumber = IPNetwork.ToBigInteger(lastIp);
            ...
            var foundIp = IPNetwork.ToIPAddress(addressNo, ipNetwork.AddressFamily);
    ...
    private static void GetPoolAddresses(IpPool pool, out IPNetwork ipNetwork, out IPAddress firstIp,
        out IPAddress? lastIp)
    {
        ipNetwork = IPNetwork.Parse(pool.IpNetwork);
