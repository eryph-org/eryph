...
public class NetworkConfigValidator(IStateStore stateStore, ILogger log) : INetworkConfigValidator
{
        ...
        NetworkProvider[] networkProviders)
    {
        ...
        foreach (var network in savedNetworks)
        {
            ...
            log.LogDebug(
                "environment '{env}', network '{networkName}': Network is in use ({countOfCatletPorts} ports) - cannot remove network.",
                network.Environment,
        ...
        {
            var network = savedNetworks.Find(x =>
                x.Environment == networkConfig.Environment && x.Name == networkConfig.Name);
            if (network == null)
                continue;

            var subnetConfigs = networkConfig.Subnets ?? [];
            ...
            {
                yield return
                    $"environment '{networkConfig.Environment}', network '{networkConfig.Name}': could not find network provider '{providerName}'";
                continue;
            }
            ...
            var countOfCatletPorts = network.NetworkPorts.Count(x => x is CatletNetworkPort);
            var environmentMessage = networkConfig.Environment == "default"
                ? ""
            ...
            if (countOfCatletPorts > 0)
            {
                ...
                {
                    log.LogDebug(
                        "environment {env}, network '{networkName}': Detected unsupported change of provider port.",
            ...
            foreach (var subnet in network.Subnets?.ToArray() ?? [])
            foreach (var ipPool in subnet.IpPools)
            {
                await stateStore.LoadCollectionAsync(ipPool, x => x.IpAssignments);
                ...
                var poolConfig =
                    (subnetConfig?.IpPools ?? [])
                    .FirstOrDefault(x => x.Name == ipPool.Name);
            ...
            foreach (var subnetConfig in subnetConfigs)
            {
                ...
                foreach (var ipPoolConfig in subnetConfig.IpPools ?? [])
                {
                    log.LogTrace("Validating ip pool config: {@config}", ipPoolConfig);
                    ...
                    if (ipPoolConfig.LastIp != ipPool.LastIp)
                    {
                        var maxIp = ipPool.IpAssignments
                            ...
                            .OfType<string>()
                            .Select(ip => IPNetwork2.ToBigInteger(IPAddress.Parse(ip)))
                            .Max();

                        var lastIpNo =
                            IPNetwork2.ToBigInteger(IPAddress.Parse(ipPoolConfig.LastIp ?? maxIp.ToString()));
    ...
    public IEnumerable<string> ValidateConfig(ProjectNetworksConfig config, NetworkProvider[] networkProviders)
    {
        ...
        foreach (var networkConfig in networkConfigs)
        {
            ...
            var environmentNetworkName =
                GetEnvironmentName(networkConfig.Environment, networkConfig.Name ?? "default");
            ...
            {
                yield return
                    $"{environmentMessage}network '{networkConfig.Name}': could not find network provider '{providerName}'";
                continue;
            }
            ...
            if (networkProvider.Type == NetworkProviderType.Flat)
            {
                if (providerIpPoolName != "default" || providerSubnetName != "default")
                    yield return
                        $"{environmentMessage}network '{networkConfig.Name}': provider subnets and ip pools are not supported for flat networks.";
            }
            else
            {
                ...
                {
                    yield return
                        $"{environmentMessage}network '{networkConfig.Name}': provider subnet '{providerName}/{providerSubnetName}' not found";
                    continue;
                }
                ...
                var overlappingNetworks = string.Join(',', ipNetworksOfNetworks
                    .Where(x => x.Network !=
                                GetEnvironmentName(networkConfig.Environment, networkConfig.Name ?? "default")
                                && x.Address is not null && networkIpNetwork.Overlap(x.Address))
                ...
                foreach (var subnetConfig in networkConfig.Subnets ?? [])
                {
                    ...
                    foreach (var poolConfig in subnetConfig.IpPools ?? [])
                    {
                        ...
                        if (!string.IsNullOrWhiteSpace(poolConfig.NextIp))
                        {
                            ...
                            if (nextIp != null && lastIp != null &&
                                IPNetwork2.ToBigInteger(lastIp) < IPNetwork2.ToBigInteger(nextIp))
                                yield return
