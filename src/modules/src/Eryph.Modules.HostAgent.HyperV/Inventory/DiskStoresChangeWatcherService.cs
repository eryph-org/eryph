...
#pragma warning restore CA2016
    ...
    public async Task Restart()
    {
        ...
        try
        {
            ...
            _subscription = ObserveStores(paths).Subscribe(
                _ => { },
                ex => logger.LogCritical(
                    ex,
                    "Failed to monitor file system events for the disk stores. Inventory updates might be delayed until eryph is restarted."),
                () => logger.LogCritical(
    ...
    /// </remarks>
    private IObservable<Unit> ObserveStores(Seq<string> paths) =>
        paths.ToObservable()
            .Select(ObservePath)
            .Merge()
