...
public class ClientService(
    ...
    IIdentityDbRepository<ClientApplicationEntity> repository)
    : BaseApplicationService<ClientApplicationEntity, ClientApplicationDescriptor>(applicationManager, repository),
        IClientService
{
    protected override async ValueTask PopulateApplicationFromDescriptor(ClientApplicationEntity application,
    ...
    protected override async ValueTask PopulateDescriptorFromApplication(ClientApplicationDescriptor descriptor,
    ...
    protected override ISingleResultSpecification<ClientApplicationEntity> GetSingleEntitySpec(string clientId,
        Guid tenantId)
    {
    ...
    private static JsonWebKeySet? CreateJsonWebKeySet(string? certificateBase64)
    {
        ...
        using (certificate)
        {
            var key = JsonWebKeyConverter.ConvertFromX509SecurityKey(
                new X509SecurityKey(certificate), true);
