...
namespace Haipa.Modules.Controller.IdGenerator
{
    ...
    ///     <list type="bullet">
    ///         <item>
    ///             <description>time - 41 bits (millisecond precision w/ a custom epoch gives us 69 years)</description>
    ///         </item>
    ///         <item>
    ///             <description>
    ///                 configured machine id - 10 bits (5 bit worker id, 5 bits datacenter id) - gives us up to 1024
    ///                 machines
    ///             </description>
    ///         </item>
    ///         <item>
    ///             <description>
    ///                 sequence number - 12 bits - rolls over every 4096 per machine (with protection to avoid
    ...
    public class Id64Generator
    {
        ...
        {
            while (true) yield return GenerateId();
        }
        ...
        private long NextId()
        {
            ...
            if (timestamp < lastTimestamp)
                throw new InvalidOperationException(string.Format(
                    "Clock moved backwards. Refusing to generate id for {0} milliseconds", lastTimestamp - timestamp));

            if (lastTimestamp == timestamp)
            {
                sequence = (sequence + 1) & SequenceMask;
                if (sequence == 0) timestamp = TillNextMillis(lastTimestamp);
            }
            else
            {
                sequence = 0;
