    ...
    : IComponentCertificateStore
{
    ...
    public void Save(byte[] clientPkcs8PrivateKey, byte[] serverPkcs8PrivateKey, ComponentEnrollmentResult result)
    {
        ...
        if (result.ServerCertificate is { Length: > 0 })
        {
            ...
            WritePfxFromResult(ServerPfxPath, result.ServerCertificate, serverPkcs8PrivateKey,
                result.ServerIssuingChain);
    ...
    public X509Certificate2? LoadClientCertificate()
    {
        ...
        if (File.Exists(PfxPath))
            return X509CertificateLoader.LoadPkcs12(
                File.ReadAllBytes(PfxPath), null);
    ...
    public (X509Certificate2 Leaf, X509Certificate2Collection Chain) LoadServerCertificate()
    {
        ...
        foreach (var certificate in bundle.Where(certificate => !ReferenceEquals(certificate, leaf)))
        ...
        if (File.Exists(PfxPath))
            try
            {
                ...
                using var leaf = X509CertificateLoader.LoadPkcs12(
                    File.ReadAllBytes(PfxPath), null, X509KeyStorageFlags.EphemeralKeySet);
