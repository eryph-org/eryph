...
public sealed class NetworkConfigValidatorTests : IDisposable, IAsyncDisposable
{
    ...
    public async Task Cannot_change_provider_port_of_used_network()
    {
        ...
        messages.Should()
            ...
            .And.Contain(
                "network 'used_network': To change the network first remove all ports or move them to another network.");
    }

    [Fact]
    public async Task Cannot_change_subnet_of_used_network()
    {
        _projectConfig.Networks.Find(x => x.Name == "used_network").IfSome(c => c.Subnets =
        [
            new NetworkSubnetConfig
            {
                Address = "192.168.15.0/24",
    ...
    public void Config_reports_use_of_ipv6()
    {
        ...
        messages.Should()
            .Contain("network 'unused_network': network address '2607:f0d0:1002:51::4' is not a IPV4 address'");
    }
    ...
    public void Config_reports_network_cidr_mismatch()
    {
        ...
        messages.Should()
            .Contain(
                "network 'unused_network': Invalid network address '192.168.10.0/8' - network cidr match network '192.0.0.0/8'");
    }
    ...
    public void Config_reports_invalid_subnet_address_mismatch()
    {
        ...
        messages.Should()
            .Contain(
                "subnet 'unused_network/default': network address '192.168.20.0/24' is not a subnet of '192.168.10.0/24'");
    }
    ...
    public void Config_reports_subnet_cidr_mismatch()
    {
        ...
        messages.Should()
            .Contain(
                "subnet 'unused_network/default': Invalid network address '192.168.10.1/24' - network cidr match network '192.168.10.0/24'");
    }
    ...
    public void Config_reports_pool_ips_mismatch()
    {
        ...
        messages.Should()
            .Contain(
                "ip pool 'pool_network/default/pool2': ip address '192.168.1.1' is not in subnet '192.168.15.0/24'")
            .And.Contain(
                "ip pool 'pool_network/default/pool2': ip address '192.168.2.1' is not in subnet '192.168.15.0/24'")
            .And.Contain(
                "ip pool 'pool_network/default/pool2': ip address '192.168.3.1' is not in subnet '192.168.15.0/24'");
    }
