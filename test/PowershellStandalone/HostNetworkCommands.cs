...
{
    public Aff<RT, TypedPsObject<CimSession>> GetCimSession() =>
        (from psEngine in default(RT).Powershell.ToAff()
        from session in psEngine.GetObjectsAsync<CimSession>(
                PsCommandBuilder.Create().AddCommand("New-CIMSession")).ToAff()
        select session)
        .Bind(x => x.HeadOrLeft(Errors.SequenceEmpty).ToAff());

    public Aff<RT, Unit> RemoveCimSession(TypedPsObject<CimSession> cimSession) =>
        from psEngine in default(RT).Powershell.ToAff()
        from vmSwitches in psEngine.GetObjectsAsync<CimSession>(
            PsCommandBuilder.Create()
                .AddInput(cimSession.PsObject)
                .AddCommand("Remove-CIMSession")
        ).ToAff()
        select Unit.Default;


    public Aff<RT, Seq<VMSwitch>> GetSwitches(TypedPsObject<CimSession> cimSession) =>
        from psEngine in default(RT).Powershell.ToAff()
        from vmSwitches in psEngine.GetObjectsAsync<VMSwitch>(
            PsCommandBuilder.Create().AddCommand("Get-VMSwitch")
                .AddParameter("CimSession", cimSession.PsObject)
            ).ToAff()
        select vmSwitches.Map(s => s.Value);

    public Aff<RT, Seq<VMSwitchExtension>> GetSwitchExtensions(TypedPsObject<CimSession> cimSession) =>
        from psEngine in default(RT).Powershell.ToAff()
    ...
    public Aff<RT, Seq<HostNetworkAdapter>> GetPhysicalAdapters(TypedPsObject<CimSession> cimSession) =>
        from psEngine in default(RT).Powershell.ToAff()
    ...
    public Aff<RT, Seq<string>> GetAdapterNames(TypedPsObject<CimSession> cimSession) =>
        from psEngine in default(RT).Powershell.ToAff()
        from netAdapters in psEngine.GetObjectsAsync<HostNetworkAdapter>(
            PsCommandBuilder.Create()
                .AddCommand("Get-NetAdapter")
    ...
    public Aff<RT, Seq<NetNat>> GetNetNat(TypedPsObject<CimSession> cimSession) =>
        from psEngine in default(RT).Powershell.ToAff()
        from netNat in psEngine.GetObjectsAsync<NetNat>(
            PsCommandBuilder.Create().AddCommand("Get-NetNat")
                .AddParameter("CimSession", cimSession.PsObject)
